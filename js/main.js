...
function validateFileType(string) {
	return (/.+(\.jpg|\.jpeg|\.png)$/).test(string);
}
...
function checkEmail(string) {
	return (/^[a-zA-z]+\W?[a-zA-z]+@[a-zA-z\.]+\.[a-z]{2,3}$/).test(string);
}
...
function checkDomainUrl(string) {
	return (/^(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/).test(string);
}
...
function createLinksFromDomains(string) {
	...
	string.match(reg).forEach((item) => string = string.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`));
